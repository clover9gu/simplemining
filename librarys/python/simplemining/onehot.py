...
def one_hot_dataframe(data, cols, replace=False):
    ...
    """
    vec = DictVectorizer()
    mkdict = lambda row: [(col, row[col]) for col in cols]
    rowDict = [dict(e) for e in data[cols].apply(mkdict, axis=1).values]
    vecData = pandas.DataFrame(vec.fit_transform(rowDict).toarray())
